...
```powershell
ng build --configuration production --base-href "https://あなたのGitHubユーザー名.github.io/リポジトリ名/"
```

**このアプリの場合：**

```powershell
ng build --configuration production --base-href "https://rmatsuo-jp.github.io/study-english/"
```
...
**⚠️ base-hrefの注意点：**
`--base-href=/study-english/` のようにパスだけを指定すると、Windows環境では `C:/Program Files/Git/study-english/` という**ローカルのファイルパスに誤解釈されることがあります**。
必ず `https://` から始まる完全なURLを指定してください。

ビルド結果は `dist/study-english/browser/` に生成されます。
ビルド後に `dist/study-english/browser/index.html` を開き、以下のようになっているか確認してください：

```html
<base href="https://rmatsuo-jp.github.io/study-english/">
```
...
```powershell
npx angular-cli-ghpages --dir=dist/study-english/browser
```

このコマンドは以下を自動で行います：

1. `gh-pages` ブランチを作成または更新する
2. `dist/study-english/browser/` の中身を全てそのブランチにpushする
...
1. Pixel の Chrome で `https://rmatsuo-jp.github.io/study-english/` を開く
2. アドレスバーの右側にインストールアイコン（⬇️）が表示されたらタップ
   - または右上の「︙」メニュー → **「アプリをインストール」**
3. 「インストール」をタップ
4. ホーム画面に「英作文添削」アプリが追加される
...
→ `dist/study-english/browser/index.html` の `<base href>` を確認してください。
